...
@media(max-width:768px){
  ...
  .app{flex-direction:column;flex:1;}
  .main{padding:16px;padding-bottom:100px;align-items:stretch;flex:1;min-height:calc(100vh - 124px);}
  .track-box{max-width:none;}
  .search-wrap{flex-direction:column;}
  .btn-track{height:48px;width:100%;margin-top:8px;}
...
<div class="app">
  ...
  <aside class="sidebar">
    ...
    <nav class="sb-nav">
      ...
      <a href="${pageContext.request.contextPath}/track" class="sb-item active"><i data-lucide="search" style="width:16px;"></i> Track Complaint</a>
  ...
  <main class="main">
    <div class="track-box">
      ...
      <div class="search-card">
        ...
        <form action="${pageContext.request.contextPath}/track" method="GET" class="search-wrap">
          <input type="text" name="id" placeholder="e.g. TCK-1234" value="<%= request.getParameter("id") != null ? request.getParameter("id") : "" %>">
          <button type="submit" class="btn-track">Track</button>
        </form>
        <% if (request.getAttribute("error") != null) { %>
          <div style="color:#D93025;margin-top:16px;font-size:13px;font-weight:600;padding:12px;background:#FFEBEB;border-radius:6px;"><%= request.getAttribute("error") %></div>
        <% } %>
      </div>

      <% 
         com.civicsathy.model.Complaint complaint = (com.civicsathy.model.Complaint) request.getAttribute("complaint");
         if (complaint != null) {
      %>
      <div class="timeline-card">
        <div class="tc-header">
          <div class="tc-id">Ticket #<%= complaint.getTrackingId() %></div>
          <div class="badge"><%= complaint.getStatus() %></div>
        </div>

        <div class="tl">
          <%
             java.util.List<com.civicsathy.model.StatusHistory> timeline = 
               (java.util.List<com.civicsathy.model.StatusHistory>) request.getAttribute("timeline");
             if (timeline != null && !timeline.isEmpty()) {
               for (int i = 0; i < timeline.size(); i++) {
                 com.civicsathy.model.StatusHistory th = timeline.get(i);
                 boolean isLast = (i == timeline.size() - 1);
                 
                 String icon = "check";
                 String dotClass = "done";
                 
                 if (isLast) {
                   if ("RESOLVED".equals(th.getNewStatus()) || "CLOSED".equals(th.getNewStatus())) {
                     dotClass = "done";
                   } else {
                     dotClass = "active";
                     icon = "IN_PROGRESS".equals(th.getNewStatus()) ? "truck" : "clock";
                   }
                 }
          %>
          <div class="t-item">
            <div class="t-dot <%= dotClass %>"><i data-lucide="<%= icon %>" style="width:12px;"></i></div>
            <div class="t-title"><%= th.getNewStatus() %></div>
            <div class="t-desc"><%= th.getComment() != null ? th.getComment() : "" %></div>
            <div class="t-time"><%= new java.text.SimpleDateFormat("MMM dd, hh:mm a").format(th.getCreatedAt()) %></div>
          </div>
          <%
               }
             }
...
<nav class="m-bnav">
  ...
  <a href="${pageContext.request.contextPath}/track" class="mn active"><i data-lucide="compass" style="width:20px;"></i>Track</a>
